...
-- src/lib/notion.ts uses the `service_role` key (bypasses RLS) and
-- src/lib/db.ts connects directly to Postgres as the table owner (also
-- bypasses RLS). So enabling RLS with no policies doesn't break app access —
-- it just stops these tables from being world-readable/writable through the
-- public API, which is what Supabase's security advisor flagged
