...
export default async function LoginPage({
  ...
  if (session) {
    ...
    if (role === "Logistics" || role === "Decorador") redirect("/guide");
    ...
    <main className="min-h-screen flex items-center justify-center bg-[#111010] p-4">
      <div className="w-full max-w-sm">
        <div className="bg-white rounded-2xl shadow-2xl p-8 space-y-7">
          ...
          {params.verify === "1" ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-sm text-green-800 text-center space-y-1">
              <p className="font-semibold">Verifica o teu email</p>
              <p className="text-green-700">Enviámos um link de acesso para o teu endereço de email.</p>
            </div>
          ) : (
            <>
              {params.error && (
              ...
              >
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-gray-300 placeholder:text-gray-400"
                />
                <button
                  type="submit"
                  className="w-full bg-white border border-gray-200 hover:bg-gray-50 transition-colors rounded-xl px-4 py-3 text-sm font-semibold text-gray-700 shadow-sm"
                >
