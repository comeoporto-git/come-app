...
import { logExpenseAction, createFornecedorAction } from "@/actions/transactions";
...
export function AddStandaloneExpenseButton({ fornecedores }: { fornecedores: Fornecedor[] }) {
  ...
  function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    setImageFile(file);
    if (file.type === "application/pdf") { setImageDataUrl("pdf"); return; }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const existing = fornecedores.find((f) => f.name.toLowerCase() === form.supplier.toLowerCase());
      if (existing) {
        fornecedorId = existing.id;
      } else if (creatingNew) {
        const newF = await createFornecedorAction(form.supplier);
      ...
      setOpen(false);
      // reset
      setForm({ supplier: "", date: new Date().toISOString().slice(0, 10), invoiceId: "", taxFree: 0, iva6: 0, iva13: 0, iva23: 0, totalCost: 0, paymentMethod: "Cartão COME", notes: "" });
      setQuery("");
      setImageFile(null);
      setImageDataUrl("");
    } catch {
  ...
  if (!open) {
    return (
      <button
        ...
        className="bg-[#32373c] text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-[#1a2018] active:scale-[0.98] transition-all"
      >
        + Despesa Avulso
      </button>
    );
      ...
      <div className="w-full bg-white rounded-t-3xl max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 pb-8 pt-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-900">Nova Despesa (sem serviço)</h2>
          ...
          <div className="space-y-3">
            ...
            {imageDataUrl === "pdf" ? (
              <div className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 bg-gray-50">
                ...
                <p className="text-sm font-medium text-gray-700 truncate flex-1">{imageFile?.name}</p>
                <button onClick={() => { setImageDataUrl(""); setImageFile(null); }} className="text-xs text-gray-400">✕</button>
              </div>
            ) : imageDataUrl ? (
              <div className="relative">
                <img src={imageDataUrl} alt="Fatura" className="w-full max-h-48 object-contain rounded-xl border border-gray-200" />
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => cameraRef.current?.click()} className="flex items-center justify-center gap-2 border border-dashed border-[#667470]/40 rounded-xl py-3 text-sm text-[#667470] bg-[#667470]/5 hover:bg-[#667470]/10 transition-colors font-medium">
                  📷 Tirar Foto
                </button>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  value={query}
                  onChange={(e) => { setQuery(e.target.value); update("supplier", e.target.value); setShowList(true); setCreatingNew(false); }}
                ...
                {showList && (
                  <ul className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-40 overflow-y-auto">
                    {filtered.map((f) => (
                      <li key={f.id}>
                        <button type="button" onMouseDown={() => { update("supplier", f.name); setQuery(f.name); setShowList(false); }}
                          className="w-full text-left px-3 py-2.5 text-sm text-[#32373c] hover:bg-[#667470]/10">
                          {f.name}
                        </button>
                      </li>
                    ))}
                    {query.length > 0 && !fornecedores.find((f) => f.name.toLowerCase() === query.toLowerCase()) && (
                      <li>
                        <button type="button" onMouseDown={() => { update("supplier", query); setShowList(false); setCreatingNew(true); }}
                          className="w-full text-left px-3 py-2.5 text-sm text-[#667470] hover:bg-[#667470]/10 font-medium">
                          + Criar &quot;{query}&quot;
                        </button>
                      </li>
                    )}
